...
V tem poglavju analiziramo odvisnost moči brezžičnega signala (\texttt{RSSI}) od oddaljenosti ter vpliv števila aktivnih naprav v prostoru.

Vse meritve so bile izvedene večkrat z namenom zmanjšanja vpliva naključnih odstopanj. Vsaka posamezna meritev je bila ponovljena petkrat, končna vrednost \texttt{RSSI} pa je bila izračunana kot povprečje teh meritev.

Za zajem podatkov smo uporabili Python skripto z uporabo knjižnice \texttt{Bleak}, ki omogoča skeniranje \texttt{BLE} naprav.

\subsection{Moč signala v prostoru}
Na Sliki \ref{fig:heatmap} je prikazana toplotna karta (angl. \texttt{heatmap}) jakosti signala v prostoru dimenzij 3 × 3,5 m. Opazimo lahko, da je moč signala največja v bližini dostopne točke ter postopoma upada z večanjem oddaljenosti. Na moč signala imajo velik vpliv tudi ovire v prostoru in odboji od sten, saj vidimo območja kjer signal nenadoma upade.
\begin{figure}[H]
	\centering
...
Na Sliki \ref{fig:stnaprav} je prikazana odvisnost \texttt{RSSI} od oddaljenosti in števila naprav. Rezultati kažejo jasno negativno korelacijo med \texttt{RSSI} in oddaljenostjo, kar je skladno s teoretičnimi modeli širjenja radijskih valov.

Vpliv števila naprav na \texttt{RSSI} pa ni tak, kot bi morda pričakovali. Iz grafa je razvidno, da se \texttt{RSSI} pri povečevanju števila naprav ne zmanjšuje monotono, temveč se vrednosti naključno spreminjajo. To lahko pripišemo več dejavnikom:

\begin{itemize}
	\item \texttt{RSSI} meri moč sprejetega signala, ne pa kakovosti povezave ali zasičenosti kanala,
	\item dodatne naprave vplivajo predvsem na zasedenost medija (interferenco, kolizije), ne nujno neposredno na izmerjeno moč signala,
	\item merilni šum in variabilnost okolja (premiki, odboji, večpotno širjenje) lahko povzročijo nihanja meritev.
\end{itemize}
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{stNaprav_heatmap.png}
	\caption{vpliv števila naprav in oddaljenosti na \texttt{RSSI}}
	\label{fig:stnaprav}
\end{figure}

Na podlagi rezultatov lahko zaključimo, da je oddaljenost glavni dejavnik, ki vpliva na upad \texttt{RSSI}, medtem ko število naprav v opazovanem eksperimentu ni imelo sistematičnega vpliva na izmerjeno moč signala, temveč predvsem na variabilnost meritev.
...
V analizirani komunikaciji z napravo OPPO Find X5 takšnih paketov nismo zaznali, kar kaže na pravilno in standardno implementacijo protokola. Vendar pa sem med testiranjem na napravi Samsung Galaxy Ace S5830 opazil pojav \texttt{Malformed Packet} pri \texttt{Echo Response}, kar nakazuje na morebitne omejitve ali nepravilnosti v starejši Bluetooth implementaciji te naprave.
...
Naloga je potrdila začetno hipotezo: Bluetooth je izjemno priročen, a hkrati varnostno tvegan protokol. Za povprečnega uporabnika je ključno zavedanje, da Bluetooth vmesnik predstavlja odprta vrata v napravo, ki jih je treba odpirati preudarno in le takrat, ko je to nujno potrebno. Pridobljeno znanje bo služilo kot osnova za nadaljnje raziskovanje brezžičnih tehnologij in kibernetske varnosti.
