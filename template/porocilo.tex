...
Naslednja vrstica v Pythonu:
\begin{verbatim}
	results = await BleakScanner.discover(timeout=5.0, return_adv=True)
\end{verbatim}
vrne slovar najdenih naprav, iz katerega odčitamo \texttt{RSSI} iskane naprave.

\subsection{Moč signala v prostoru}
Na sliki \ref{fig:heatmap} je prikazana toplotna karta (angl. heatmap) jakosti signala v prostoru dimenzij 3 × 3,5 m. Dostopna točka, označena z modro, se nahaja med točkama A2 in A3. Opazimo, da je signal najmočnejši v njeni bližini ter postopoma slabi z naraščajočo oddaljenostjo. Na porazdelitev moči pomembno vplivajo tudi ovire in odboji od sten, kar se kaže v območjih z nenadnimi padci signala. Predmeti, ki najbolj vplivajo na moč signala so označeni na sliki.
\begin{figure}[H]
	\centering
...
Ali se \texttt{RSSI} spremeni, če v okolico naše bluetooth povezave dodajamo naprave, na katere generiramo bluetooth promet?
\begin{verbatim}
	\\Ukaz za generiranje prometa
	sudo l2ping <MAC address>
\end{verbatim}
Bluetooth uporablja t.i. \texttt{adaptive frequency hopping (AFH)}, kar pomeni, da naprave, ob pošiljanju paketov, izjemno hitro spreminjajo frekvenčne kanale (BLE uporabljajo 40 kanalov) in se izogibajo trkom in šumu.
Zaradi tega se \texttt{RSSI} ne spremeni. Razlika ob povečanju prometa je ta, da se zaradi trkov izgubi več paketov, posledično pa pride do večje latence.
